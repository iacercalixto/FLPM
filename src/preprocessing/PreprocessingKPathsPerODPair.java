 ...
 * 
 * For each OD pair in the model, find exactly 'k' routes for each.
 * The 'k' value is to be given by the user. Instead of calculating all the
 * costs for all the k routes, the costs will be calculated for k = 1.
 * 
 * For k > 1, the are two options (according to another parameter given by the user):
 * - the cost of the route will be normally calculated as it is for
 * routes with k==1;
 * - the cost of the route will be the value of the least cost route
 * between the OD pair under consideration times C, where C is a positive constant.
 * 
 * @author Iacer Calixto
 */
public class PreprocessingKPathsPerODPair implements IPreprocessing
{
	/**
	...
	/**
	 * Whether using or not PETGyn. If using PETGyn,
	 * the routes are generated the transformed graph G', not G
	 */
	private static boolean usingPETGyn = false;
	
	/**
	 * Constructor
	 */
	public PreprocessingKPathsPerODPair()
	{
		this.k = 5;
	}
			...
			ODCounts od, RouteCosts rc, LinkCosts lCosts, ODPairs odPairs) throws Exception
	{
		...
		HashMap<String, Node> nodes = new HashMap<String, Node>();//[ graph.vertexSet().size() ];
		HashMap<String, Edge> edges = new HashMap<String, Edge>();//[ graph.edgeSet().size() ];
		...
		while (it.hasNext())
		{
			...
			Node n = new Node( Integer.parseInt(vertex), vertex );
			nodes.put( Integer.toString( Integer.parseInt(vertex) ), n);
			
			//System.out.println("Node " + n + " added to position " + Integer.parseInt(vertex) );
		}
		...
		while (it2.hasNext())
		{
			...
			// instead of using the weight that comes in the graph,
			// use the updated weight in the LinkCosts object
			double weight = 0;// = (double)graph.getEdgeWeight(e);
			if (lCosts.getCountsBetween(v1, v2)!=null)
				weight = Double.parseDouble( lCosts.getCountsBetween(v1, v2) );
			
			// Alter the weight in the graph
			((WeightedGraph)graph).setEdgeWeight(e, weight);
			
			//System.out.println("edge: "+e+", weight: "+weight);
			
			Edge e2 = new Edge(
					nodes.get(Integer.toString( Integer.parseInt(v1))),
					nodes.get(Integer.toString( Integer.parseInt(v2))),
					weight,
					0,
					0
			);
			edges.put( Integer.toString(counter2++), e2);
			
			//System.out.println("Edge " + e2 + " added to position " + counter2);
		}
		...
		while(itODPairs.hasNext() /*&& boolLooping*/)
		{
			...
			// Get a list of the (maximum) k paths starting in 'v1' and
			// ending in 'v2'
			...
			if (newPaths != null)
			{
				...
				while (itNewPaths.hasNext())
				{
					...
					for (Object edge : (entry.getEdgeList()))
					{
						...
						Edge e2 = new Edge(
								nodes.get(Integer.toString(Integer.parseInt(vFrom))),
								nodes.get(Integer.toString(Integer.parseInt(vTo))),
								(int)weight, 0, 0);
						
						for (int i=0; i<edges.size(); i++)
						{
							if (edges.get( Integer.toString(i) ).equals(e2))
								e2 = edges.get( Integer.toString(i) );
						}
					...
					if (counterTemp>0)
					{
						...
						if (intPathCounter==1)
						{
							...
							//System.out.println("minimum cost route: " + p1 + ": " +
							//		Double.toString(((GraphPath)newPaths.get(0)).getWeight()) );
						} else
						{
							// If the path is NOT a minimum-cost path (k>1) 
							if (this.useConstant)
							{
								// If constant C is being used
								// the path cost equals the k==1 path times C
								p.put(p1, Double.toString(((GraphPath)newPaths.get(0)).getWeight() * this.c) );
								//System.out.println("NOT minimum cost route: " + p1 + ": " +
								//		Double.toString(((GraphPath)newPaths.get(0)).getWeight() * this.c) );
							} else
							{
								// If constant C is NOT being used,
								// the path cost equals its total cost
		...
		if (!usingPETGyn)
		{
			// Sanitize the generated routes.
			// If any route repeats a node, remove it.
			Iterator <?> itRoutes = p.entrySet().iterator();
			while (itRoutes.hasNext())
			{
				Map.Entry entry = (Map.Entry) itRoutes.next();
				
				boolean routeContainsRepeatedNode = false;
				
				ArrayList arrRoute = (ArrayList) entry.getKey();
				String strRoute = arrRoute.toString();
				
				String[] strNodes = strRoute.split(",");
				for (int i=0; i<strNodes.length; i++)
				{
					for (int j=0; i<strNodes.length; i++)
					{
						// Skip the node under evaluation
						if (i==j) continue;
						
						// Clean the node entry (reference node)
						strNodes[i] = strNodes[i].replace("[", "").replace("]", "").replace(" ", "");
						
						// Clean the node entry (node to compare to the reference one)
						strNodes[j] = strNodes[j].replace("[", "").replace("]", "").replace(" ", "");
						
						if ( strNodes[i].equalsIgnoreCase(strNodes[j]) )
							routeContainsRepeatedNode = true;
					}
				}
				
				// If the route contains at least one repeated node
				// remove it from the return hashmap
				if (routeContainsRepeatedNode)
				{
					itRoutes.remove();
			...
			while (itRoutes.hasNext())
			{
				...
				for (int i=0; i<strNodes.length; i++)
				{
					for (int j=0; i<strNodes.length; i++)
					{
						...
						int iNodeInG = f.getCorrespondentNodeInG( Integer.parseInt(strNodes[i]) );
						int jNodeInG = f.getCorrespondentNodeInG( Integer.parseInt(strNodes[j]) );
						
						//System.err.println(iNodeInG+" "+jNodeInG);
						
						if ( iNodeInG == jNodeInG)
							routeContainsRepeatedNode = true;
					}
				}
				...
				{
					itRoutes.remove();
					System.err.println("Route "+strRoute+
							" removed.");
				}
			}
		}
	...
	public void setUsingPETGyn(Boolean b) throws Exception
	{
		this.usingPETGyn = (boolean)b;
	}
}
